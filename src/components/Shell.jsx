...
export default function Shell() {
  ...
  const isMainView = ['/home', '/map', '/messages', '/profile', '/recommendations'].includes(location.pathname)

  return (
    <div className="app-shell">
      <div className="mobile-frame">
        <header className="topbar">
          <button className="avatar top-avatar" onClick={() => navigate('/profile')} aria-label="Open profile">{user.avatar}</button>
          <div className="topbar-copy">
            <span className="eyebrow">SmartSync</span>
            <h1>{isMainView ? 'SmartSync' : title}</h1>
          </div>
          <div className="topbar-meta">
            <button className="icon-button notification-button" onClick={() => navigate('/notifications')} aria-label="Notifications">
              <Bell size={20} />
        ...
        <button className="fab-create" onClick={() => navigate('/create')} aria-label="Create activity">
          <Plus size={26} />
        </button>
        ...
        <nav className="bottom-nav" aria-label="Primary navigation">
          {tabs.map(({ to, label, icon: Icon }) => (
            <NavLink key={to} to={to} className={({ isActive }) => isActive ? 'nav-item active' : 'nav-item'}>
              <span className="nav-icon-wrap"><Icon size={21} /></span>
              <span>{label}</span>
            </NavLink>
          ))}
        </nav>
