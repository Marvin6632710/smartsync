...
**Live in `d88393b`, pushed and deployed to Firebase Hosting after localhost
owner review.** `/messages` now uses a medium-width inbox layout: title and
subtitle, one compact search/filter toolbar, then one bordered conversation list with
...
Hosting serves checksum-matched `index-B2Rs0uVj.js` and
`index-CVU3-Yd0.css`; the live `/messages` route returns HTTP 200.
...
now use the normal own-message bubble with a small spinner beside the time.
The visible **Checking** title and long sentence are gone, as is the permanent
moderation sentence under the composer; the checking description remains for
screen readers. Delivered own messages show a subtle check beside their time.

On the server, one Admin SDK `getAll` preflight reads the existing message,
activity, role and sender profile. Typed-text moderation, image moderation and
image transcription start concurrently; when OCR finds words, their moderation
is the dependent second stage. That means a captioned picture with OCR may use
four OpenAI requests in total, with the first three overlapped. The OpenAI
8-second abort now includes response-body reading and JSON parsing.

The boundary has not moved: no client can write a message or chat picture, and
the Function creates no message, picture or chat notification before all
required moderation has allowed it.

Release verification: **957 unit/app and 402 rules tests pass**; lint,
Prettier, Maps configuration and the production build are clean. The local chat
layout was visually checked. Production lists the callable as active, and the
served JS/CSS checksums match the local build. The signed-in live chat opens
with delivered checks and no permanent moderation sentence; no new production
message was submitted during verification.

## Current release — expanded admin powers (2026-09-24)

**Live in `d7d8595`, pushed and deployed to Functions, Firestore rules and
Hosting.** The approved seven-part console expansion is implemented:
individual profile picture/bio/username moderation; activity-picture
moderation; removal/restoration of a published message; timed suspension with
automatic expiry; user appeals with uphold/reverse decisions; session
revocation and server-side password-reset delivery; and expiring announcements
for all users, hosts or participants.

Sensitive actions are callable Functions in `functions/index.js` backed by
`functions/lib/admin.js`. Removed public content is preserved in server-only
`moderationVault`; public `contentModeration` locks stop an owner bypassing the
removal; every action is reasoned, audited and notified. `moderationAppeals`
and `announcements` have read-only client rules, while Functions own every
write. Timed suspension expiry is enforced directly in rules and cleaned by
the scheduled `expireTimedSuspensions` Function every fifteen minutes.
Password reset looks up the email and calls Identity Toolkit only on the
server, so no email is exposed to the console; localhost uses the Auth emulator
endpoint and sends no real email. New routes: user `/appeals`, admin
`/admin/appeals` and `/admin/announcements`. ADR-035 and the latest HANDOFF
section contain the design and file map.

Verification for that deployed admin release, before the current chat
refinement: **952 unit/app, 402 rules, 8 Auth/Firestore integration, 11
push-delivery and 3 push-trigger tests pass**. Lint, formatting, Maps
configuration and the production build were clean. The seeded browser
walkthrough of the new admin and user appeal surfaces is complete; no
destructive action was submitted. Production lists all six new Functions,
rules released, and the signed-in live `/admin` dashboard shows Appeals and
Recent announcements. Served JS/CSS checksums match that release's local
build.
...
## Chat moderation — calibrated and live (2026-09-23)
...
The three chat callables and their rules are live. Production uses the configured
`OPENAI_API_KEY`; localhost keeps the stand-in option described in HANDOFF.md.
...
half-finished implementation to resume. Repository files carry the handoff; do
not assume access to the previous assistant's conversation.

## Current release

- Repository: <https://github.com/Marvin6632710/smartsync>, branch `main`.
- The latest application release is `6b323f6`; it is on `origin/main`.
- Live site: <https://smartsync-c1f07.web.app>.
- Deployed chat release: `6b323f6`, entry bundle `index-4OXYldmG.js` and CSS
  bundle `index-iC2Ysgsx.css` (both checksum-matched against the public site).
- Hosting includes Google Maps, picture uploads and name-only search.
  Firestore rules were last released with expanded admin powers, `d7d8595`;
  indexes remain deployed from the picture-upload release.
- The current Hosting release adds the quieter moderated-chat send state; the
  shared activity calendar and alarm-style time picker remain included.
...
| Minimum age 15 (ADR-034)                               | **LIVE 2026-09-23 — `bc49f2c`, pushed, rules + hosting deployed.** A **date of birth** (not an age — an age is true for a year and would go stale in the one number the minimum depends on) is asked at sign-up and, for accounts made before the gate, on a screen above every other route except a closed account. It lives in the PRIVATE profile and is never public. **The rules enforce it**, not the form: a private-profile write carrying a date under the minimum is refused, with the cut-off built as a `YYYY-MM-DD` string from `request.time` because rules have no date arithmetic. The derived **age** may appear on the public profile, off by default; turning it off sets the field to `null` rather than hiding it at render time (Firestore has no field-level read rules — ADR-005's argument again). The owner's own card shows it either way, labelled "only you". The refusal screen offers no retry on purpose. `src/utils/age.js` is the whole policy and `MIN_AGE` the one number to change; new files `AgeCheckPage.jsx`, `TooYoungScreen.jsx`, `tests/unit/age.test.js`, `tests/app/ageGate.test.jsx`. `pendingSignUpName` → `pendingSignUpDetails` (returns `{name, dateOfBirth}`) so the date survives the sign-up/observer race. 28 new tests + 5 rules tests. **Self-declared and unverified** — README §13 says so. **Before deploying: every existing live account, demo ones included, is asked once on next entry.**                                                                                                                                                                                                                  |
| Chat moderation (ADR-033)                              | **Live 2026-09-24; latest refinement `6b323f6`.** The database refuses every client-written chat message (`allow create: if false` on `activities/*/messages` and `chatPictures/{messageId}`), so the only writer is `sendChatMessageCall`, which re-checks membership/suspension/closure/retention in `gate()` and then moderates — that is what makes the check unskippable. Text and pictures go to OpenAI's `omni-moderation-latest`; a picture is also transcribed by `gpt-5.6-luna` and the transcription moderated as text, because the API applies `hate`/`harassment` to text only. Nine categories block, each needing the API boolean **and** a score over a floor in `functions/lib/moderation.js`; `violence`, `illicit`, `self-harm` and `self-harm/intent` deliberately do not. A refusal stays in the sender's composer with a general reason and Edit / Discard / Ask for a review; appeals reach `/admin/chat`, where Overturn posts the original from the held copy and deletes it. `sexual/minors` is blocked with nothing retained and no appeal (README §11). Outages keep the message and offer Retry — failure is never delivery. Rate limit 60/hour/person plus a daily send-check budget. Moderation API is free; only transcription is billed (~$0.0007/picture). The latest release batches the preflight reads, overlaps independent moderation work and uses a compact spinner/check send state. New: `functions/lib/{moderation,openai,chat,sendChat}.js`, `src/firebase/chat.js`, `src/console/pages/ChatBlocksPage.jsx`, `scripts/fake-openai.mjs`. Verified end-to-end in the emulator and deployed with its configured `OPENAI_API_KEY`. |
...
| Age gate                   | `src/utils/age.js` (the whole policy), `src/App.jsx` (the gate), `src/pages/AgeCheckPage.jsx`, `src/components/TooYoungScreen.jsx`, `src/pages/SignUpPage.jsx`, `src/firebase/auth.js`, `src/firebase/users.js`, `src/pages/PrivacyPage.jsx`, `src/pages/ProfilePage.jsx`, `firestore.rules` |
...
Current fast results: **957 unit/app tests and 402 rules tests**. The last
auxiliary runs recorded **8 Auth/Firestore integration, 11 push-delivery and 3
push-trigger tests** passing. Lint, source formatting, build and Maps
configuration checks passed. Existing build warnings concern Firebase chunk
size and picture-module chunking.
...
the owner's complete picture save/reload/mobile walkthrough; real Maps
light-theme and device GPS permission flows. Historical
