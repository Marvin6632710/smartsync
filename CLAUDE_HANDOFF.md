...
| Idea | Current state |
| --- | --- |
| Full-width profile separate from activities on laptops | Desktop still uses a 360 px profile column beside activities. The full-width layout remains a possible next task. |
| More noticeable “Current Warnings” in Settings | Its link still uses the ordinary settings-row treatment. Increased visual prominence remains a possible next task. |
| Bio below the profile picture, up to 75 words | Bio already exists and is displayed. The editor and Firestore rules currently limit it to **300 characters**, not 75 words. A requested change must reconcile both constraints and all supported languages. |
| Unread count on the notification bell | Already implemented in `Shell.jsx` for mobile and desktop using notifications whose `read` value is false. Verify before proposing any replacement. |
