...
## Current working tree — moderated chat latency and send state (2026-09-24)

**Uncommitted, not deployed, and ready for localhost testing.** Pending sends
...
required moderation has allowed it. Resume with the localhost send-path and
visual pass; do not call this refinement committed or deployed.
...
layout was visually checked without the permanent moderation sentence and with
delivered checks; the owner still needs to send a fresh local message to judge
the shortened pending transition.
...
The owner is switching assistants because of session limits. The deliberate
uncommitted chat refinement above is the implementation to resume and is ready
for localhost testing. Repository files carry the handoff; do not assume access
to the previous assistant's conversation.
